...
public class PhysAgent extends SpaceAgent
{
	...
	public  PhysAgent(double radius, int shape)
	{
		super(radius, shape);
		_init();
		_createPhysAgent();
	...
	public void applyCollisionForce(SpaceAgent a)
	{
		...
		d.mul(((r / dist) * TestModel.collisionCoefficient));
		this.force.add(d);
	}
	...
	public void applySeparationForces()
	{
		...
		if (_result != null)
		{
			for (int _i = 0; _i < _result.size(); _i++)
			{
				...
				f.add((new Vector((new Vector(v)).div(Math.pow(d, 2.0)))).mul(TestModel.separationCoefficient));
			}
		}
		this.applyForce(f);
	}
	...
	{
		if ((TestModel.collisionCoefficient > 0.0))
		{
			this.applyCollisionForces();
		}
		if ((TestModel.separationCoefficient > 0.0))
		{
			this.applySeparationForces();
		}
		this.integrate(dt);
		this.velocity.mul(TestModel.velocityCoefficient);
		this.clearForces();
	}
	
	public void step(SimulationTime time)
	{
		this.applyForce((new Vector((new Vector(this.getPosition().normalize())).negate())).mul(TestModel.centralForceCoefficient));
	}
