...
class pc(general_cell):
    ...
    def growth(self):
        ...
        """
        if self.stage == 6:                 # Increase original size by 1/4
            self.area *= 1.25
        elif self.stage == 12:               # Decrease by 1/4 to achieve original, then increase by 2/4
            sa = self.area / 1.25
            self.area = sa * 1.5
        elif self.stage == 18:               # Decrease by 2/4 to achieve original, the increase by 3/4
            sa = self.area / 1.5
            self.area = sa * 1.75
